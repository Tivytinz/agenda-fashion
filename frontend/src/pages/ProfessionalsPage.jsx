          ...
          {items.map((professional) => {
            ...
            return (
              <article className="management-card professional-card" key={professional.id}>
                <div className="account-avatar">{professional.foto_url ? <img alt="" src={professional.foto_url} /> : <span>{String(professional.nome || "P").slice(0, 1)}</span>}</div>
      ...
      <dialog className="cancel-dialog" ref={removeDialogRef}>
        <div className="cancel-dialog-content">
          ...
          <div className="cancel-dialog-actions">
            <button className="button button-secondary" onClick={() => removeDialogRef.current?.close()} type="button">Manter profissional</button>
