...
describe("checkout PIX", () => {
  ...
  it("não anuncia plano ativo enquanto o pagamento confirmado ainda está ativando", async () => {
    mockInitialization();
    apiRequest
      .mockResolvedValueOnce({
    ...
    expect(screen.getByText(/Pagamento confirmado. Estamos ativando seu plano/)).not.toBeNull();
    expect(screen.queryByRole("heading", { name: "Plano e assinatura" })).toBeNull();
  });

  it("avisa quando o pagamento foi confirmado mas a ativação requer atenção", async () => {
    mockInitialization();
    apiRequest
      .mockResolvedValueOnce({
    ...
    expect(
      screen.getByText(/a ativação precisa de atenção/i)
    ).not.toBeNull();
    expect(
      screen.getByText(/Não gere outro PIX/i)
    ).not.toBeNull();
    expect(
      screen.queryByRole("heading", { name: "Plano e assinatura" })
    ).toBeNull();
  });
