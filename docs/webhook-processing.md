...
Além do lease por evento, a reserva serializa eventos que compartilham o mesmo par `(provedor, recurso_id)`. A operação usa um advisory lock transacional curto do PostgreSQL apenas durante o ato de reservar e revalida que não exista outro registro do mesmo recurso em `PROCESSING`. O lock termina no fim da própria instrução de reserva; ele não permanece aberto durante processamento do domínio nem durante chamadas HTTP ao Asaas. Eventos sem `recurso_id` continuam independentes e usam o próprio ID apenas como chave interna da reserva.

Essa serialização por recurso impede que, por exemplo, `PAYMENT_CONFIRMED` e `PAYMENT_RECEIVED` da mesma cobrança iniciem simultaneamente dois efeitos externos de ativação. Ela reduz concorrência operacional, mas não substitui idempotência nem a validação de ordem no domínio financeiro.
