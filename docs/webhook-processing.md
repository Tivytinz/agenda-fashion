...
`Subscribe` da Meta mantém `event_id` estável no formato `subscribe:<assinatura_id>`. O evento `purchase` do Google mantém `transaction_id` estável no formato `af-subscription-<assinatura_id>`. O horário da conversão também é reconstruído a partir de `pagamentos.data_pagamento`: Meta recebe esse instante em `event_time` e Google Measurement Protocol em `timestamp_micros`, em vez de atribuir ao retry o horário em que a fila finalmente conseguiu enviar. Esses identificadores e o horário original complementam a outbox local: como uma chamada HTTP externa pode ter sido aceita antes de uma queda impedir a confirmação local, a entrega é operacionalmente *at-least-once* e depende também das chaves estáveis para deduplicação no provedor.
