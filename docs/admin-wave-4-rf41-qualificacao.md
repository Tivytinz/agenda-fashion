...
- `scripts/performance-admin-audit.mjs` mede as três consultas superadmin:
  recentes, pendentes e por ator (três aquecimentos, 30 amostras, concorrência
  3). É somente leitura. Com `PERF_ADMIN_BUILD_SHA`, identifica o commit.
- `scripts/performance-admin-audit-qa.mjs` mede criação de campanha auditada e
  ...
  superadmin desse banco e SHA do build sob teste. Não aceita URL de produção.
...
Guardar a saída JSON dos **cinco** cenários com horário, SHA do build,
...
- **CA-ADM-13:** p95 ≤ 2 s para as três leituras, criação auditada e revisão,
