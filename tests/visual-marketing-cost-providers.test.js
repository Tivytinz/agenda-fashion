...
describe("Pinterest Ads", () => {
  test("testa conexão sem colocar o token na URL", async () => {
    ...
    expect(url).toBe(
      "https://api.pinterest.com/v5/ad_accounts/549768356618"
    );
    expect(options.headers.Authorization).toBe("Bearer pinterest-token");
    expect(url).not.toContain("pinterest-token");
    expect(JSON.stringify(result)).not.toContain("pinterest-token");
  });

  test("lista campanhas reais com paginação e remove arquivadas", async () => {
    global.fetch
    ...
    const result = await providers.listarCampanhas("pinterest_ads");
  ...
  test("converte gasto em micro moeda para centavos por dia", async () => {
    global.fetch
      .mockResolvedValueOnce(response({
        items: [
          {
    ...
    expect(analyticsUrl).toContain("campaigns%2Fanalytics").not;
  ...
  test("não consulta Pinterest se a configuração estiver incompleta", async () => {
    process.env.PINTEREST_ADS_ACCESS_TOKEN = "";

    await expect(
      providers.testarConexao("pinterest_ads")
...
describe("TikTok Ads", () => {
  test("testa conexão pelo advertiser configurado e usa Access-Token", async () => {
    global.fetch.mockResolvedValueOnce(response({
      code: 0,
      message: "OK",
      data: {
  ...
  test("lista campanhas e normaliza metadados", async () => {
    global.fetch.mockResolvedValueOnce(response({
      code: 0,
      message: "OK",
      data: {
    ...
    const result = await providers.listarCampanhas("tiktok_ads");
  ...
  test("converte spend do relatório em centavos", async () => {
    global.fetch
      .mockResolvedValueOnce(response({
        code: 0,
        message: "OK",
    ...
    const reportUrl = global.fetch.mock.calls[1][0];
    expect(reportUrl).toContain("/report/integrated/get/");
    expect(reportUrl).toContain("AUCTION_CAMPAIGN");
    expect(reportUrl).toContain("spend");
  });

  test("trata erro semântico da API mesmo quando o HTTP é 200", async () => {
    global.fetch.mockResolvedValueOnce(response({
      code: 40001,
