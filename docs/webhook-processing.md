...
Além da idempotência por `evento_id`, eventos do mesmo recurso Asaas são serializados. A coluna `evento_criado_em` preserva o `dateCreated` do envelope do provedor como `TIMESTAMP WITHOUT TIME ZONE`; esse valor não representa um instante financeiro do AF e é usado somente para comparar a ordem relativa de eventos produzidos pelo mesmo provedor. Quando já existe na fila um evento mais recente para o mesmo `recurso_id`, um evento antigo que ainda esteja `PENDING`, `FAILED` elegível a retry ou `PROCESSING` abandonado é concluído como `IGNORED` antes de executar regra financeira. Um evento em processamento ativo não é invalidado no meio da execução; o evento mais novo aguarda a exclusão mútua do recurso e depois reconcilia o estado final.

A reconciliação considera também o estado atual informado no objeto `payment`: um evento de sincronização cujo pagamento já esteja `CONFIRMED`, `RECEIVED` ou `RECEIVED_IN_CASH` pode ativar a assinatura mesmo quando o nome do evento não for o evento original de confirmação. Da mesma forma, `PAYMENT_CHARGEBACK_REQUESTED`, `PAYMENT_CHARGEBACK_DISPUTE` e `PAYMENT_AWAITING_CHARGEBACK_REVERSAL` mantêm o acesso suspenso até que um evento posterior de confirmação/recebimento represente a reversão efetiva. Isso impede que entrega fora de ordem ou retry atrasado rebaixe ou reative o plano com um estado antigo.
