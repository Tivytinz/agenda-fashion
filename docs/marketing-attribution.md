...
A recorrência também é quebrada por **origem de aquisição**, reutilizando exatamente a classificação de atribuição já adotada pelo backend do funil profissional. Campanhas resolvidas oficialmente permanecem separadas por origem canônica, enquanto `organico`, `rastreamento_incompleto`, `identidade_nao_oficial` e `sem_evidencia` continuam categorias distintas. Um sinal de recorrência nunca promove tráfego incompleto para atribuição oficial e não reescreve a evidência capturada. Para cada origem, o painel mostra chegada ao primeiro, segundo e terceiro agendamento e reaplica D7, D14 e D30 somente sobre profissionais maduros naquela própria origem.
